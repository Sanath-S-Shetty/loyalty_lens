...
export default function ReportsView({ brand }) {
  ...
  const handleShareReport = async () => {
    // 1. Format a clean, readable summary for Slack/Teams/Email
    const summaryText = `📊 Loyalty Intelligence Report: ${brand.name.toUpperCase()}

🏆 Program Score: ${brand.score}/100
📈 Market Position: ${brand.marketPosition}
💬 Customer Sentiment: ${brand.sentiment?.positive || 0}% Positive

💡 Executive Summary:
${brand.summary?.executiveSummary || "Analysis complete."}

Generated by LoyaltyLens.`;

    // 2. Try the modern Web Share API (Triggers native iOS/Android/Windows share menus)
    if (navigator.share) {
      try {
        await navigator.share({
          title: `${brand.name} Intelligence Report`,
          text: summaryText,
        });
      } catch (error) {
        // Users frequently cancel the share menu, which throws an error. We can just ignore it.
        if (error.name !== 'AbortError') {
          console.error("Error sharing natively:", error);
        }
      }
    } 
    // 3. Fallback for Desktop Browsers (Copies to Clipboard)
    else if (navigator.clipboard) {
      try {
        await navigator.clipboard.writeText(summaryText);
        alert("📋 Executive summary copied to your clipboard! You can now paste it into Slack or an email.");
      } catch (error) {
        console.error("Failed to copy text:", error);
        alert("Failed to copy the report. Please try again.");
      }
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white/[0.03] backdrop-blur-md border border-white/[0.06] rounded-3xl p-6 shadow-2xl space-y-3">
            ...
            <button className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-2xl bg-gradient-to-r from-teal-500 to-emerald-500 hover:from-teal-400 hover:to-emerald-450 text-black font-extrabold text-xs transition-all duration-300 shadow-lg shadow-teal-550/10 cursor-pointer    "  onClick={() => {
    // Pass the active JSON object directly to the generator
    generatePDF(brand); 
  }}>
            ...
            <button className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-2xl border border-white/10 hover:bg-white/5 text-white font-semibold text-xs transition duration-200 cursor-pointer" onClick={handleShareReport}
  className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-semibold py-3 px-4 rounded-xl transition duration-200">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
